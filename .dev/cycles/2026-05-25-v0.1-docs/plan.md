...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] 涵蓋專案定位、目標讀者、技術棧（Flutter / Riverpod / llama.cpp / MLX / Pigeon / FFI）
  - [x] 列出 v0.1 主要能力與支援平台矩陣
  - [x] Quick start：clone → fvm flutter pub get → llama.cpp build → run（指向 setup.md）
  - [x] 連結到 CONTRIBUTING / CHANGELOG / docs/architecture / docs/adr
  - [x] 含 license、致謝（llama.cpp / mlx-swift-lm 等）
- **預估時間**: 0.5 天
...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] 開發流程：fork / branch / PR / review / merge
  - [x] Commit 規範：Conventional Commits（type/scope/subject + body + footer）
  - [x] 編碼規範：Dart 風格、Riverpod 命名、測試策略（unit / widget / integration 邊界）
  - [x] PR 模板要求與 Issue 模板要求
  - [x] 連回 setup.md 與 architecture/overview.md
- **預估時間**: 0.5 天
...
- **驗收標準**:
  - [x] 文件遵循 Keep a Changelog 與 SemVer 慣例的標頭
  - [x] v0.0.4 至 v0.1.0（unreleased）區段齊備
  - [x] v0.1.0 區段以 Added / Changed / Fixed / Removed 分類覆蓋 EP1-EP6 重大變更
  - [x] 涉及 breaking change 的條目明確標註
- **預估時間**: 0.5 天
...
- **建議方式**: 大綱 → 圖 → 文字 → 審閱
- **驗收標準**:
  - [x] 分層圖：UI / ViewModel / Controller / Service / Engine（FFI + Pigeon bridge）/ Native
  - [x] 資料流圖：使用者輸入 → ChatTemplate → GenerationController → Engine → token stream → UI
  - [x] 列出 core 模組及其職責、入口、擴充點（engine、profile、template、platform、download 等）
  - [x] 標明 v0.1 引入的關鍵抽象（ChatTemplate / ModelProfile / GenerationController）
  - [x] 章節結尾連到對應 ADR
- **預估時間**: 1 天
...
- **建議方式**: 變更 → 驗證 → 部署
- **驗收標準**:
  - [x] `docs/adr/README.md` 列出 ADR 索引（編號、標題、狀態）
  - [x] 標準 ADR 章節：Status、Context、Decision、Consequences、Alternatives Considered
  - [x] 索引能讓讀者快速找到任一 ADR
- **預估時間**: 0.5 天
...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] Context 說明為何 v0.1 引入 MLX
  - [x] Decision 明確記錄選 Path A
  - [x] Alternatives：列出 B / C，包含優劣與被否決原因
  - [x] Consequences：streaming callback / Swift Package 依賴 / iOS-only 等後果
  - [x] 引用素材：exploration.md C 線（line 225+）
- **預估時間**: 0.5 天
...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] Context 說明 llama.cpp 與 MLX 的整合介面差異
  - [x] Decision 記錄雙軌並存，並界定各自適用場域
  - [x] Consequences：複雜度 / 維護成本 / 後續是否能收斂為單軌
  - [x] Alternatives：統一走 FFI 或統一走 Pigeon 的代價
- **預估時間**: 0.5 天
...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] Context 說明 streaming 的特殊性（vs request-response）
  - [x] Decision 記錄採用的 pattern（含程式示意）
  - [x] Consequences：cancel 行為、錯誤傳遞、背壓
  - [x] Alternatives：其他 streaming 模式被否決原因
- **預估時間**: 0.5 天
...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] Context 說明舊 PromptFormat 的痛點 + Smoking Gun
  - [x] Decision 記錄 ChatTemplate + Resolver 設計
  - [x] Consequences：擴充新模型族的方式、向後相容性
  - [x] Alternatives：保留 PromptFormat / 走 jinja2-like template engine 等被否決方案
- **預估時間**: 0.5 天
...
- **建議方式**: 草稿 → 審閱 → 定稿
- **驗收標準**:
  - [x] Context 說明 ViewModel 過胖的舊狀態
  - [x] Decision 記錄哪些責任進 Controller（生成、metrics、cancel）、哪些留 VM（UI 狀態）
  - [x] Consequences：可測試性、metrics 收集點、未來新增 backend 的影響
  - [x] Alternatives：留在 VM / 引入 BLoC 等被否決
- **預估時間**: 0.5 天
...
- **驗收標準**:
  - [x] 環境需求：macOS 版本、Xcode、fvm + Flutter 版本、CocoaPods、Pigeon
  - [x] llama.cpp build：對應 `scripts/llama.cpp_MacOS_Build.md` 的步驟與產出 static lib
  - [x] iOS / macOS 各自的 Runner 設定（framework 連結、entitlements、code signing 提示）
  - [x] 模型下載與放置位置
  - [x] 常見錯誤排查（依賴版本衝突如 pigeon ^22.7.2 vs riverpod 3.x）
  - [x] 文件中所有指令可實際執行
- **預估時間**: 1 天
...
- **驗收標準**:
  - [x] core 模組（engine / template / profile / controller）public API 皆有 dartdoc 註解
  - [x] 非對外擴充點 class / method 標記 `@internal`
  - [x] `fvm dart doc` 在本地能產出 site，無 unresolved doc reference warning
  - [x] CONTRIBUTING 補一段「如何加 @internal / 寫 dartdoc」說明
- **預估時間**: 1-1.5 天
