...
- 2026-08-11 本票 Comments 記載的整合方式已被取代：ticket 07 的 review finding 2 修正後，服務不再使用 `lib/ui/shared/inference/generation_controller.dart` 的 `GenerationController`（那是 service 反向依賴 UI layer），改為服務自行宣告 `FoodReligionTextGenerator` 並預設直接跑 `InferenceSession`。此 feature 從未使用 controller 的 metrics，seam 仍留著讓任何 runner adapt 進來。見 commit `4fe8bc9`。
