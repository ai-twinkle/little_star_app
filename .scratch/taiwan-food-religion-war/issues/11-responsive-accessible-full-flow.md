...
- Automated responsive coverage completes the replacement flow at 320 × 568, 430 × 932, and 1280 × 900. A 320 × 568 defense run with a 280 px keyboard-equivalent inset and 50 visible characters verifies the fixed challenge, count, missing-model state, model entry, and submit action remain naturally reachable.
- Added 2× text-scaling and screen-reader coverage for progress, card selection/locking, belief slate, draw, fixed challenge, live character count, inline error, waiting, verdict, fallback disclosure, result actions, and removal of stale result semantics after replay.
- Added system-Back cancellation coverage at choice, draw, defense, judging, and result, followed by confirmed exit. Existing tests continue to cover model-manager round trips, rediscovery, cancelled downloads, reduced motion, first-event-only selection/draw/submit, and late-result disposal.
