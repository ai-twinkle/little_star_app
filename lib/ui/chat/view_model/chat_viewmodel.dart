...
class ChatViewModel extends ChangeNotifier {
  ...
  String get systemPrompt =>
      _settings.systemPrompt ?? 'You are a helpful AI assistant.';
  ...
  Future<void> sendMessage(String text) async {
    ...
    await _sub?.cancel();
    _messages.add(
      ChatMessage(content: text, isUser: true, modelName: selectedModelName),
    );
    notifyListeners();
  ...
  Future<void> stopGeneration() async {
    ...
    if (streamingMessageNotifier.value.isNotEmpty) {
      ...
      _messageMetrics[_messages.length - 1] = MessageMetrics(
        stopReason: StopReason.cancelled.name,
      );
    }
  ...
  Future<void> _generateResponse() async {
    ...
    final contextMessages =
        _messages.length > _contextMessageCount
